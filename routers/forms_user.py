...
# ---------- Endpoints (existing) ----------
# ... (everything you already have remains unchanged below until the end)
...
def get_saved_answers(
    ...
    answers_kv = {q_by_step.get(r.step_id, f"step:{r.step_id}"): r.value for r in rows}
...
def save_section_answers(
    ...
    if getattr(body, "answers_kv", None):
        for qid, raw in body.answers_kv.items():
            ...
            incoming[s.id] = _validate_and_normalize(s, raw)
    if getattr(body, "answers", None):
        for item in body.answers:
            ...
            incoming[item.step_id] = _validate_and_normalize(s, item.value)
    ...
    for step_id, normalized in incoming.items():
        row = existing.get(step_id)
        if row:
            row.value = normalized
        else:
            db.add(UserStepAnswer(progress_id=p.id, step_id=step_id, value=normalized))
    ...
    rows = db.query(UserStepAnswer).filter(UserStepAnswer.progress_id == p.id).all()
    answers_kv = {q_by_step.get(r.step_id, f"step:{r.step_id}"): r.value for r in rows}
    rem_rows = db.query(UserStepReminder).filter(UserStepReminder.progress_id == p.id).all()  # <-- NEW
    reminders_kv = {
        q_by_step.get(r.step_id, f"step:{r.step_id}"): bool(r.wants_reminder)
        for r in rem_rows
...
def list_entries(category_id: int, section_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    for p in rows:
        ...
        kv = {q_by_step.get(r.step_id, f"step:{r.step_id}"): r.value for r in items}

        rem_items = db.query(UserStepReminder).filter(UserStepReminder.progress_id == p.id).all()  # <-- NEW
        rkv = {
            q_by_step.get(r.step_id, f"step:{r.step_id}"): bool(r.wants_reminder)
            for r in rem_items
            if bool(r.wants_reminder)
...
def list_all_reminders(
    ...
    val_by_key = {(a.progress_id, a.step_id): a.value for a in ans_rows}
...
def list_progress_leaves(
    ...
    """
    Return leaves for a progress. By default we now return ALL leaves (active/accepted/removed)
    so the frontend can compute counts and avoid showing 'Leaf: incomplete' after decisions.
    Set include_removed=false to get only ACTIVE ones.
    """
...
def replace_progress_leaves(
    ...
    """
    Replace the ACTIVE set of leaves with given contact_ids.
    - Add any missing as active (reactivate if previously removed)
    - Remove any extra contacts (hard delete for simplicity)
    """
    p = _get_owned_progress_or_404(db, user.id, progress_id)
