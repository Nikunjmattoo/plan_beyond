...
from sqlalchemy import or_, func, cast  # add cast
from sqlalchemy.dialects.postgresql import JSONB  # add JSONB
...
# ------------------------ Invitee-side routes (Branch To-Do) ------------------------
@router.get("/branches/todo")
def my_branch_todo(db: Session = Depends(get_db), user: Depends(get_current_user) = Depends(get_current_user)):
...
def trigger_collection_release(
    ...
    # ⛔️ For time-based collections, only allow manual trigger once due/past
